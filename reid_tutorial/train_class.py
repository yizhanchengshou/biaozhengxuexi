...
# Datasets
parser.add_argument('--root', type=str, default='D:/xiangmu/reid/data', help="root path to data directory")
...
parser.add_argument('--train-batch', default=15, type=int,
                    help="train batch size")
parser.add_argument('--test-batch', default=15, type=int, help="test batch size")
...
def test(model, queryloader, galleryloader, use_gpu, ranks=[1, 5, 10, 20]):
    ...
    if not args.test_distance == 'global':
        ...
        from reid_tutorial import low_memory_local_dist
    ...
    if args.reranking:
        from reid_tutorial import re_ranking
...
def main():
    ...
    # 学习率
    if args.stepsize > 0:
        scheduler = lr_scheduler.StepLR(optimizer, step_size=args.stepsize, gamma=args.gamma)  #  阶梯型衰减
